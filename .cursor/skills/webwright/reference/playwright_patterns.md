...
These are the canonical heredoc patterns the Webwright agent uses. In Cursor
you run them via the Shell tool — no JSON wrapping, no escaping gymnastics,
just one shell command per turn.
...
For visual checks, read individual PNG files inside
